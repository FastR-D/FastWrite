...
export interface CodexAdapterOptions { command?: string; args?: string[]; cwd?: string; env?: NodeJS.ProcessEnv }
...
export class CodexHarnessAdapter implements HarnessAdapter {
  ...
  async #run(input: SendMessageInput, queue: AsyncEventQueue<HarnessEvent>): Promise<void> {
    ...
    try {
      const result = await this.#request("turn/start", { threadId: input.session.sessionId, input: input.content, ...(input.skills ? { skills: input.skills } : {}) }, input.signal);
    queue.push({ type: "run.completed", runId, result }); queue.close();
