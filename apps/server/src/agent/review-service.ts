...
export class ReviewService {
  ...
  async run(projectId: string, sourceOnly = false, requestSignal?: AbortSignal, pdfPageText: string[] = []): Promise<ReviewResponse> {
    ...
    try {
      ...
      const issues: ReviewIssue[] = result.issues.filter(Boolean).map((issue, index) => ({
        id: `issue_${crypto.randomUUID()}`,
        reportId,
        category: ["novelty", "clarity", "correctness", "related-work", "compliance", "reproducibility"].includes(issue.category) ? issue.category : "clarity",
        severity: ["blocking", "major", "minor"].includes(issue.severity) ? issue.severity : "minor",
        priority: severityPriority(["blocking", "major", "minor"].includes(issue.severity) ? issue.severity : "minor") * 100 + index,
        title: String(issue.title ?? "Review finding").trim(), rationale: String(issue.rationale ?? "Generated by Harness review.").trim(), impact: String(issue.impact ?? "May affect the manuscript quality.").trim(), suggestion: String(issue.suggestion ?? "Inspect and revise this finding.").trim(),
        evidence: (Array.isArray(issue.evidence) ? issue.evidence : []).map((evidence) => {
