...
export function PublicationTargetFields({ profile, value, onChange, onSelectedVenueChange }: PublicationTargetFieldsProps) {
  ...
  const chooseVenue = (venueId: string) => {
    ...
    onChange({ domain: profile, venueId: venue.value, stage: "submission", ...(venue.tracks?.[0] ? { track: venue.tracks[0].value } : {}) });
  };
  ...
  return <>
    <label className="field">
      ...
      <select value={value?.venueId ?? ""} onChange={(event) => chooseVenue(event.target.value)}>
        ...
        <optgroup label="CCF-A conferences">{venues.filter((venue) => venue.kind === "conference").map((venue) => <option key={venue.value} value={venue.value}>{venue.label}</option>)}</optgroup>
        <optgroup label="CCF-A journals">{venues.filter((venue) => venue.kind === "journal").map((venue) => <option key={venue.value} value={venue.value}>{venue.label}</option>)}</optgroup>
      </select>
