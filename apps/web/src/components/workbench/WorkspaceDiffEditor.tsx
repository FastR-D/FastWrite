...
export function WorkspaceDiffEditor({ registry, projectId, projectVersion, dirty, request, onClose, onRestored, onSaveCheckpoint }: { registry: DocumentRegistry; projectId: string; projectVersion: number; dirty: boolean; request: DiffRequest; onClose: () => void; onRestored: (path: string) => Promise<void>; onSaveCheckpoint: () => Promise<number> }) {
  ...
  return <section className="workspace-diff" aria-label="History comparison" data-diff-target={request.targetRef} data-diff-path={request.path}>
    <div className="workbench-actions"><strong>{request.oldPath ? `${request.oldPath} → ` : ""}{request.path}</strong><select aria-label="Diff layout" value={mode} onChange={event => setMode(event.target.value as typeof mode)}><option value="auto">Automatic layout</option><option value="split">Side by side</option><option value="inline">Inline</option></select><button disabled={!changeCount} onClick={() => jump(-1)}>Previous change</button><button disabled={!changeCount} onClick={() => jump(1)}>Next change</button><span>{changeCount} changes</span><button disabled={(request.targetRef === "working" || request.targetRef === "working-tree") || !comparison?.modified.exists || comparison.modified.binary || restoring} onClick={() => setConfirmRestore(true)}>Restore file</button><button disabled={restoring} onClick={onClose}>Close comparison</button></div>
    {confirmRestore ? <div className="diff-restore-confirmation" role="group" aria-label="Confirm history restore"><p>Replace {request.path} with checkpoint {request.targetRef.slice(0, 8)}. A new history commit will record this restore.</p>{dirty ? <p>Local edits must be saved to a checkpoint before restoring.</p> : null}<button disabled={dirty || restoring} onClick={() => void restore(false)}>Confirm restore</button><button disabled={restoring} onClick={() => void restore(true)}>Save checkpoint then restore</button><button disabled={restoring} onClick={() => setConfirmRestore(false)}>Cancel</button></div> : null}
