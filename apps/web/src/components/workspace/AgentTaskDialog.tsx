...
import { AlertTriangle, Check, ChevronRight, FileText, LoaderCircle, RotateCcw, ShieldCheck, X } from "lucide-react";
...
export function AgentTaskWorkspace({ open, project, seed, compileState, onRequestCompile, onClose, onAccepted }: { open: boolean; project: PaperProject; seed: AgentTaskSeed; compileState: CompileStateReport; onRequestCompile: () => void; onClose: () => void; onAccepted: (path: string) => void | Promise<void> }) {
  ...
  useEffect(() => {
    ...
    if (stage === "input") setObjective(seed.objective ?? "");
  ...
  const footer = (
    ...
    stage === "input" ? <><Button variant="ghost" onClick={() => setObjective((value) => value.trim() ? value : "/draft ")}>Draft</Button><Button variant="primary" disabled={!objective.trim()} onClick={() => void createPlan()}>Create plan</Button></> :
  ...
  const content = <>
    {stage === "input" ? <div className="agent-task-form"><label className="field"><span>What should Agent do?</span><textarea value={objective} onChange={(event) => setObjective(event.target.value)} placeholder="Create an initial draft, continue the TODO sections, or revise the Introduction and Evaluation consistently…" autoFocus /></label><small className="agent-command-hint">Optional intent commands: /draft, /continue, /revise.</small>{seed.path ? <small className="agent-command-hint">Editor context: {seed.path}</small> : null}{seed.issueIds?.length ? <div className="agent-issue-seed"><ShieldCheck /> Revision is linked to {seed.issueIds.length} Review Issue{seed.issueIds.length === 1 ? "" : "s"}.</div> : null}</div> : null}
