...
export function AgentTaskWorkspace({ open, project, seed, compileState, onRequestCompile, onClose, onAccepted }: { open: boolean; project: PaperProject; seed: AgentTaskSeed; compileState: CompileStateReport; onRequestCompile: () => void; onClose: () => void; onAccepted: (path: string) => void | Promise<void> }) {
  ...
  const pendingHasBlocking = changeSet?.changes.some((change) => change.hunks?.some((hunk) => hunk.status === "pending" && hunk.findings?.some((finding) => finding.status === "blocking"))) ?? false;
  ...
  const decidePending = async (status: "accepted" | "rejected") => { if (!changeSet) return; const decisions = pendingDecisions(changeSet, status); if (decisions.length) await submitDecisions({ decisions }, true); };
  ...
  const editHunk = async (hunkId: string, after: string) => { if (!changeSet || !selectedChange) return; setError(""); setChangeSet(await api.revisions.edit(project.id, changeSet.id, { hunks: [{ path: selectedChange.path, hunkId, after }] })); };
  ...
  const footer = (
    ...
    stage === "diff" ? <><Button variant="ghost" disabled={deciding} onClick={onClose}>Leave review</Button>{reviewCounts?.pending ? <><Button variant="ghost" icon={<X />} disabled={deciding} onClick={() => void decidePending("rejected")}>Reject pending & complete</Button><Button variant="secondary" icon={<Check />} disabled={deciding || pendingHasBlocking} title={pendingHasBlocking ? "Edit or reject blocking hunks before accepting all pending changes" : undefined} onClick={() => void decidePending("accepted")}>Accept pending & complete</Button></> : <Button variant="primary" icon={<CheckCircle2 />} disabled={deciding} title="Finalize the accepted and rejected hunk choices" onClick={() => void finishReview()}>Complete review</Button>}</> :
