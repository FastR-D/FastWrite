...
export function AgentTaskWorkspace({ open, project, seed, compileState, onRequestCompile, onClose, onAccepted, onNavigate }: { open: boolean; project: PaperProject; seed: AgentTaskSeed; compileState: CompileStateReport; onRequestCompile: () => void; onClose: () => void; onAccepted: (path: string) => void | Promise<void>; onNavigate?: (path: string, line?: number) => void }) {
  ...
  const content = <>
    {stage === "input" ? <div className="agent-task-form"><label className="field"><span>What should Agent do?</span><textarea id="agent-task-objective" name="agent-task-objective" ref={objectiveRef} value={objective} onChange={(event) => setObjective(event.target.value)} placeholder="Create an initial draft, continue the TODO sections, or revise the Introduction and Evaluation consistently…" autoFocus /></label><div className="agent-command-buttons" aria-label="Agent intent commands">{AGENT_INTENT_COMMANDS.map(({ intent, label }) => <Button type="button" size="small" variant="secondary" icon={intent === "draft" ? <FilePlus2 /> : intent === "continue" ? <ListTodo /> : <PencilLine />} aria-pressed={activeAgentIntentCommand(objective) === intent} key={intent} onClick={() => applyIntent(intent)}>{label}</Button>)}</div>{taskSkills.length ? <label className="field"><span>Task Skills</span><select multiple value={taskSkillIds} onChange={(event) => setTaskSkillIds(Array.from(event.target.selectedOptions, (option) => option.value))}>{taskSkills.filter((skill) => !skill.supportedIntents.length || skill.supportedIntents.includes(activeAgentIntentCommand(objective) ?? "revise")).map((skill) => <option key={skill.id} value={skill.id}>{skill.id} · {skill.description}</option>)}</select></label> : null}{seed.path ? <small className="agent-command-hint">Editor context: {seed.path}</small> : null}{seed.issueIds?.length ? <div className="agent-issue-seed"><ShieldCheck /> Revision is linked to {seed.issueIds.length} Review Issue{seed.issueIds.length === 1 ? "" : "s"}.</div> : null}</div> : null}
