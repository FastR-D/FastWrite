...
export function ReviewDialog({ open, project, compileState, onRequestCompile, onClose, onNavigate, onReviseLocally, onReviseWithAgent }: ReviewDialogProps) {
  ...
  const pdfPreviewAvailable = compiledCurrentVersion && Boolean(compileState.pageText?.some((text) => text.trim()));
  const compiling = compileState.state === "loading" || compileState.state === "compiling";
  const run = async (sourceOnly: boolean) => { const controller = new AbortController(); runAbortRef.current = controller; setLoading(true); setActiveRun(null); setError(""); try { const response = await api.reviews.run(project.id, sourceOnly, controller.signal, compiledCurrentVersion && compileState.pageText?.length ? { pageText: compileState.pageText, projectVersion: project.version } : undefined); setReport(response.report); setActiveRun(response.run); setSelected([]); } catch (failure) { setError(failure instanceof DOMException && failure.name === "AbortError" ? "Review cancelled. No report was created." : message(failure)); } finally { if (runAbortRef.current === controller) runAbortRef.current = null; setLoading(false); } };
  ...
  return <Dialog open={open} width={width} className="review-dialog" resizable={width === "wide"} title="Paper Review" description={`${project.skill.name} · saved ${snapshotTime(project.updatedAt)}`} headerActions={<div className="dialog-size-controls" aria-label="Review window size"><IconButton label="Compact review window" icon={<Minimize2 />} aria-pressed={width === "large"} onClick={() => setWidth("large")} /><IconButton label="Wide review window" icon={<Maximize />} aria-pressed={width === "wide"} onClick={() => setWidth("wide")} /><IconButton label="Fullscreen review window" icon={<Maximize2 />} aria-pressed={width === "fullscreen"} onClick={() => setWidth("fullscreen")} /></div>} onClose={() => { if (!loading) onClose(); }} footer={<><Button variant="ghost" disabled={loading} onClick={onClose}>Close</Button>{!compiledCurrentVersion && !loading ? <Button variant="secondary" loading={compiling} disabled={compiling} onClick={onRequestCompile}>{compiling ? "Compiling" : "Compile current version"}</Button> : null}{loading ? <Button variant="secondary" onClick={() => runAbortRef.current?.abort()}>Cancel review</Button> : <Button variant="primary" icon={<ShieldCheck />} onClick={() => void run(!pdfPreviewAvailable)}>{report ? pdfPreviewAvailable ? "Run new review" : "Run source-only review" : pdfPreviewAvailable ? "Review paper" : "Continue source-only"}</Button>}</>}>
    ...
    {loading ? <div className="agent-progress review-run-progress"><LoaderCircle className="spin" /><strong>Reviewing the frozen paper snapshot</strong><span>Collecting section evidence, applying the Writing Skill, and deduplicating issues.</span>{activeRun?.steps?.length ? <ol>{activeRun.steps.map((step) => <li key={step.id} className={`is-${step.status}`}><span>{step.status === "completed" ? <CheckCircle2 /> : step.status === "failed" ? <AlertTriangle /> : step.status === "running" ? <LoaderCircle className="spin" /> : null}</span><strong>{step.label}</strong><small>{step.status.replace("-", " ")}</small></li>)}</ol> : null}</div> : report ? <div className="review-report">
      <header className="review-summary"><div><span className={`recommendation recommendation--${report.recommendation}`}>{(report.recommendation ?? "invalid-report").replace("-", " ")}</span><strong>{report.overallAssessment}</strong></div><small>{report.issues.length} issues · saved report</small></header>
      <p className="review-input-kind">{report.inputType === "pdf-preview" ? "Reviewed source and bounded PDF page text. Figures and visual layout were not inspected." : "Source-only review; PDF content was not supplied."}</p>
      {incompleteCoverage ? <div className="form-error" role="status"><AlertTriangle /> {report.recommendation ? "Review coverage is incomplete. Failed or skipped passes must be inspected before treating this report as clean." : "This saved report is incomplete. Run a new review; it cannot be treated as a clean result."}</div> : null}
      {report.passes?.length ? <section className="review-pass-coverage" aria-label="Review pass coverage"><h3>Pass coverage</h3><div>{report.passes.map((pass) => <details className={`review-pass review-pass--${pass.status}`} key={pass.id}><summary><strong>{pass.id}</strong><small>{pass.status}{pass.issues.length ? ` · ${pass.issues.length} issues` : ""}</small></summary><dl><div><dt>Provider</dt><dd>{pass.provider ?? "unavailable"}{pass.model ? ` / ${pass.model}` : ""}</dd></div><div><dt>Input</dt><dd>{pass.inputBoundary ?? "not recorded"}</dd></div>{pass.error || pass.unavailableReason ? <div><dt>Reason</dt><dd>{pass.error ?? pass.unavailableReason}</dd></div> : null}</dl></details>)}</div></section> : null}
