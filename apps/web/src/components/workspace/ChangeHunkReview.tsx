  ...
  return <div className="hunk-review">
    ...
    <div className="hunk-review__list">{hunks.map((hunk, index) => {
      ...
      return <article className={`hunk-card hunk-card--${hunk.status}`} key={hunk.id}>
        ...
        {editing ? <div className="hunk-editor"><TextArea aria-label={`Edit hunk ${index + 1} for ${change.path}`} value={draft} onChange={setDraft} spellCheck={false} autoFocus />{error ? <div className="form-error" role="alert">{error}</div> : null}<footer><Button size="small" variant="ghost" icon={<Icon name={icons.close} />} disabled={saving} onClick={() => { setEditingHunk(null); setDraft(""); setError(""); }}>Cancel edit</Button><Button size="small" variant="primary" icon={<Icon name={icons.check} />} loading={saving} disabled={saving || draft === hunk.after || draft === hunk.before} onClick={() => void saveEdit()}>Save hunk</Button></footer></div> : <div className="revision-diff">{parts.map((part, partIndex) => part.type === "delete" ? <del key={partIndex}>{part.value}</del> : part.type === "insert" ? <ins key={partIndex}>{part.value}</ins> : <span key={partIndex}>{part.value}</span>)}</div>}
        ...
        {hunk.findings?.length ? <div className="hunk-findings">{hunk.findings.map((finding) => { const anchor = finding.anchors?.[0]; return <p className={`is-${finding.status}`} key={finding.id}><strong>{finding.status} · {finding.source}</strong><span>{finding.message}</span>{anchor && onNavigate ? <Button variant="ghost" onClick={() => onNavigate(anchor.path, anchor.prefix.split("\n").length)}>Open evidence</Button> : null}</p>; })}</div> : null}
        {!readOnly && !editing ? <footer>{onEditHunk ? <Button size="small" variant="secondary" icon={<Icon name={icons.edit} />} disabled={busy || hunk.status === "accepted"} title={hunk.status === "accepted" ? "Change to reject before editing this hunk" : "Edit only this hunk"} onClick={() => { setEditingHunk(hunk.id); setDraft(hunk.after); setError(""); }}>Edit hunk</Button> : null}{hunk.status !== "rejected" ? <Button size="small" variant="ghost" icon={<Icon name={icons.close} />} disabled={busy} onClick={() => onDecide([hunk.id], "rejected")}>{hunk.status === "accepted" ? "Change to reject" : "Reject"}</Button> : null}{hunk.status !== "accepted" ? <Button size="small" variant={hunk.status === "pending" ? "primary" : "secondary"} icon={<Icon name={icons.check} />} disabled={busy || hunk.findings?.some((finding) => finding.status === "blocking")} title={hunk.findings?.some((finding) => finding.status === "blocking") ? "Edit or reject this hunk to resolve its blocking finding" : undefined} onClick={() => onDecide([hunk.id], "accepted")}>{hunk.status === "rejected" ? "Change to accept" : "Accept"}</Button> : null}</footer> : null}
      </article>;
    })}</div>
  </div>;
}
