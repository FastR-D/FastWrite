...
export const api = {
  ...
  compiler: {
    compileOnServer: (projectId: string, signal?: AbortSignal) => request<{ success: boolean; engine: "server"; log: string; error?: string; pdfBase64?: string; syncTexData?: string; workspacePaths: string[]; pageText?: string[]; projectVersion?: number }>(`/api/projects/${projectId}/compile`, { method: "POST", ...(signal ? { signal } : {}) })
  },
  completions: {
