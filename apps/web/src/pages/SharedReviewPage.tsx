...
import { Button, Icon, icons, TextArea, TextField } from "../components/ui";
...
export function SharedReviewPage({ token }: { token: string }) {
  ...
  return <main className="shared-review-page"><header className="workspace-topbar"><div className="workspace-topbar__left"><span className="brand__mark">F</span><div className="project-identity"><strong>{data.project.name}</strong><span>Shared review · version {data.project.version}</span></div></div><span className="skill-badge"><Icon name={icons.verified} /> {data.permission === "comment" ? "Comment access" : "Read only"}</span></header><div className="shared-review-layout"><aside><h2>Files</h2>{paths.map((item) => <Button variant="ghost" className={`shared-review-file${item === path ? " is-active" : ""}`} key={item} onClick={() => void select(item)}>{item}</Button>)}</aside><section><header><strong>{path}</strong><span>Manuscript editing is disabled for shared links.</span></header><pre>{content}</pre></section><aside><h2>Comments</h2>{data.comments.filter((comment) => comment.path === path).map((comment) => <article key={comment.id}><strong>{comment.author}{comment.line ? ` · line ${comment.line}` : ""}</strong><p>{comment.body}</p><small>{new Date(comment.createdAt).toLocaleString()}</small></article>)}{data.permission === "comment" ? <div className="shared-comment-form"><TextField className="shared-field" value={author} onChange={setAuthor} placeholder="Your name" /><TextField className="shared-field" value={line} onChange={setLine} inputMode="numeric" placeholder="Line (optional)" /><TextArea className="shared-field" value={body} onChange={setBody} placeholder="Review comment" /><Button icon={<Icon name={icons.comment} />} disabled={!author.trim() || !body.trim()} onClick={() => void comment()}>Add comment</Button></div> : <p>This link is read-only.</p>}{error ? <div className="form-error">{error}</div> : null}</aside></div></main>;
}
